...
class CompletionFormatter {
  ...
  constructor(editor: monacoeditor.editor.ITextModel, position: monacoeditor.Position) {
    this._editor = editor
  ...
  private removeDuplicateStartOfSuggestions(): this {
    ...
    const before = this._editor
      .getValueInRange(new monacoeditor.Range(1, 1, this._cursorPosition.lineNumber, this._cursorPosition.column))
      .trim()
  ...
  private getNoTextBeforeOrAfter = () => {
    ...
    const textBeforeRange = new monacoeditor.Range(0, 0, this._cursorPosition.lineNumber, this._cursorPosition.column)
